...
- **What changed**:
  ...
  - **Bottom-of-Viewport Editorial Progressive Blur Overlay (Multi-Stop Cubic Falloff)**:
    - Expanded height to $140\text{px}$ on desktop and $90\text{px}$ on mobile.
    - Softened blur intensity to an optical depth of $7\text{px}$ (`backdrop-filter: blur(7px)`).
    - Engineered a 9-stop cubic ease-out alpha gradient mask (`rgba(0,0,0,1)` down to `transparent` through 8 intermediate curves) eliminating all hard edges and producing a buttery, seamless photographic focus falloff.
